...
class SpliceEventPipeline:
    def __init__(self, config):
        ...
        # self.GTFpath = self.g_config['genome_ref']
        self.retryN = 2
    ...
    def get_one_cmd(self, oneComp):
        ...
        specific_comparison = self.g_config['specific_comparison']
        majiq_cutoff_val = self.g_config['majiq_cutoff_val']
        junction_FC = self.g_config['junctionFC']
        junction_MAX = self.g_config['junctionMAX']
        sample_sheet_file = self.g_config['sample_info']
        addexons = self.g_config['addexons']
        file_prefix = os.path.join(wd_folder,oneComp)
        os.makedirs(os.path.join(file_prefix,'junction_prep'), exist_ok=True)
        # copy an empty rmats file to the output folder in case the file is not generated
        shutil.copy(os.path.join(self.strPipePath,"src/SpliceEvent","rmats_junction_prep.csv"),os.path.join(file_prefix,'junction_prep','rmats_junction_prep.csv'))
        ...
        if self.g_config['junction_filter'] and sample_sheet_file:
        ...
        if self.g_config['junction_filter'] and bam_folder:
        ...
        strCMD.append(f"python -u %s -a {annotation} -r {reference} -sj {sj_output} -o {harm_prep_dir}"%os.path.join(self.strPipePath,"src/SpliceEvent","harm_prep.py") + (" -addexons %s"%addexons if addexons else "")) # TODO: name mismatch?
        ...
        # run 3
        strCMD.clear()
        strCMD.append(f"python -u %s -indir {harm_post_dir} -outdir {final_output} -allgene {final_allgene_output} -majiq_score_cutoff {majiq_cutoff_val}"%os.path.join(self.strPipePath,"src/SpliceEvent","post_process.py"))
        ...
        if self.g_config['junction_filter']:
        ...
        if self.g_config['junction_filter']:
            ...
            strCMD.append(f'\tpython -u %s -infile $input_csv -outdir {all_junction_output} -Altbam "{altbam_files}" -Refbam "{refbam_files}" -inputjson {inputjson} -FCthreshold {junction_FC} -maxcount {junction_MAX}\ndone')
    ...
    def resume_task(self):
        ...
        if self.g_config['specific_comparison'] is None:
            return False
        ...
        self.g_config['specific_comparison'] = ",".join(rJobs)
